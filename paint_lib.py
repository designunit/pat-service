...
def draw_by_pattern_JPEG(parsed_pattern, img_size, background_color):
    ...
    origin = Vector(parsed_pattern[1], parsed_pattern[2])

    # supposed to be parsed from file
    local_pattern = PatternLine(parsed_pattern[0], origin, parsed_pattern[3],
    ...
    basis_vector.paint_image_out(origin, img_size[0], shift, drawing_object)

    reverse_vector = Vector.from_angle(math.radians(local_pattern.angle))
    reverse_vector.paint_image_out(origin, img_size[0], -shift, drawing_object)
